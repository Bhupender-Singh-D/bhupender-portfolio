  ...
  return (
    <section id="portfolio" className="py-20 relative overflow-hidden bg-slate-100/50 dark:bg-slate-900/40">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* Filter Tabs - 5px Button Radius */}
        <div className="flex flex-wrap items-center justify-center gap-2 mb-12">
          {categories.map((cat) => (
            <button
              key={cat}
              onClick={() => setSelectedCategory(cat)}
              className={`px-4 sm:px-5 py-2.5 rounded-[5px] font-medium text-xs sm:text-sm transition-all duration-200 focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:outline-none ${selectedCategory === cat
        ...
        {/* Portfolio Cards Grid - 8px Card Radius */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredProjects.map((project, idx) => (
            <ScrollReveal key={project.id} direction="up" delay={idx * 70}>
              <div
                className="glass-card rounded-[8px] overflow-hidden border border-slate-200/80 dark:border-slate-800 flex flex-col justify-between group hover:-translate-y-2 transition-all duration-300 shadow-xl h-full"
              >
                ...
                <div className="h-52 relative p-3 flex flex-col justify-between overflow-hidden bg-slate-900">
                  <img
                    src={project.imageBg}
                    alt={`${project.title} - ${project.category} project by Bhupender Singh`}
                    loading="lazy"
                    decoding="async"
                    width={400}
                    height={220}
                  ...
                  <div className="flex justify-between items-center z-10 relative">
                    <span className="px-3 py-1 rounded-[5px] text-xs font-semibold bg-slate-900/80 text-white backdrop-blur-md border border-slate-700/50 shadow-md">
                      {project.category}
                    </span>

                    <button
                      onClick={() => setActiveModalProject(project)}
                      className="p-2 rounded-[5px] bg-slate-900/80 hover:bg-slate-950 text-white backdrop-blur-md border border-slate-700/50 shadow-md transition-all hover:scale-110 focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:outline-none"
                      title="Quick Details"
                ...
                <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
                  ...
                  <div className="flex flex-wrap gap-1.5 pt-2">
                    {project.tech.map((t, tIdx) => (
                      <span
                        key={tIdx}
                        className="px-2.5 py-0.5 rounded-[5px] text-xs font-medium bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700"
                      >
                        {t}
                      </span>
                    ))}
                  ...
                  {/* Action Buttons - 5px Radius */}
                  <div className="pt-4 border-t border-slate-200/80 dark:border-slate-800/80 flex items-center justify-between gap-3">
                    <a
                      href={project.demoUrl}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-[5px] font-semibold text-xs text-white bg-gradient-to-r from-blue-600 to-purple-600 hover:opacity-95 shadow-md transition-all focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:outline-none"
                    >
                    ...
                    <button
                      onClick={() => setActiveModalProject(project)}
                      className="inline-flex items-center justify-center px-3.5 py-2.5 rounded-[5px] font-semibold text-xs text-slate-700 dark:text-slate-200 bg-slate-200/70 dark:bg-slate-800/70 hover:bg-slate-300 dark:hover:bg-slate-700 transition-colors gap-1 focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:outline-none"
                      title="View Details"
                    >
                      <span>Details</span>
      ...
      {/* Project Details Modal - 8px Container Radius */}
      {activeModalProject && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-200">
          <div className="glass-card max-w-3xl w-full rounded-[8px] overflow-hidden border border-slate-200 dark:border-slate-800 shadow-2xl relative max-h-[90vh] flex flex-col bg-white dark:bg-slate-900">
            ...
            <div className="p-4 sm:p-5 border-b border-slate-200/80 dark:border-slate-800/80 flex items-center justify-between shrink-0 bg-slate-50/50 dark:bg-slate-900/50">
              <div className="flex items-center gap-3">
                <span className="px-3 py-1 rounded-[5px] text-xs font-bold bg-blue-500/10 text-blue-600 dark:text-cyan-400 border border-blue-500/20">
              ...
              <button
                ...
                className="p-2 rounded-[5px] text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white bg-slate-200/60 dark:bg-slate-800/60 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:outline-none"
                title="Close Modal"
            ...
            <div className="p-5 sm:p-6 space-y-6 overflow-y-auto custom-scrollbar">
              ...
              <div className="relative w-full rounded-[8px] overflow-hidden border border-slate-200 dark:border-slate-800 bg-slate-950 shadow-xl">
                {/* Browser Header Bar */}
                <div className="bg-slate-900 px-4 py-2.5 flex items-center justify-between border-b border-slate-800/80">
                  ...
                  <div className="flex items-center gap-2 px-3 py-1 rounded-[5px] bg-slate-950 text-slate-400 text-xs font-mono max-w-xs sm:max-w-sm truncate border border-slate-800/80">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {activeModalProject.tech.map((t, idx) => (
                    <span key={idx} className="px-3 py-1.5 rounded-[5px] text-xs font-semibold bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-700">
                      {t}
                    </span>
                  ))}
                </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  {activeModalProject.highlights.map((item, idx) => (
                    <div key={idx} className="flex items-center gap-2.5 p-2.5 rounded-[8px] bg-slate-50 dark:bg-slate-800/40 border border-slate-200/60 dark:border-slate-700/50 text-xs sm:text-sm text-slate-700 dark:text-slate-300">
            ...
            <div className="p-4 sm:p-5 border-t border-slate-200 dark:border-slate-800 flex items-center gap-3 shrink-0 bg-slate-50/50 dark:bg-slate-900/50">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-[5px] font-semibold text-xs sm:text-sm text-white bg-gradient-to-r from-blue-600 to-purple-600 hover:opacity-95 shadow-md transition-all focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:outline-none"
              >
              ...
              <a
                href="#contact"
                onClick={() => setActiveModalProject(null)}
                className="flex items-center justify-center gap-2 px-4 sm:px-5 py-3 rounded-[5px] font-semibold text-xs sm:text-sm text-slate-700 dark:text-slate-200 bg-slate-200/80 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 transition-colors focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:outline-none"
              >
